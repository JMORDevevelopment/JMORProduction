                ...
                <div class="row justify-content-between">
                    <div class="col-lg-8">
                        @foreach($blogs as $blog)
                            <div class="blog-post">
                                <a href="{{ route('blog.detail', Str::after($blog->link, 'blog/')) }}">
                                    <img class="rounded mb-lg-4 mb-3" src="{{ asset($blog->image) }}" height="300px" alt="card image">
                                </a>
                                <h3><a href="{{ route('blog.detail', Str::after($blog->link, 'blog/')) }}">{{ $blog->name }}</a></h3>
                                <div class="meta font-lora mb-3">
                                    <a href="#">Post Date</a>
                                    <a href="#">{{ date('m-d-Y', strtotime($blog->published)) }}</a>
                                </div>
                                <p>{{ strip_tags(substr($blog->description, 0, 300)) . '....' }}</p>
                            </div>
                        @endforeach
                    </div>
                    <div class="col-lg-4">
