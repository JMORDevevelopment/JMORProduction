                            ...
                            @foreach($main_blogs as $main_blog)
                                <div class="blog-post">
                                    @if(!empty($main_blog->image))
                                        <a href="{{ route('jmor-shows.detail', $main_blog->link) }}"><img class="rounded mb-lg-4 mb-3" src="{{ asset($main_blog->image) }}" height="350px" alt="card image"></a>
                                    @else
                                        <img class="rounded mb-lg-4 mb-3 img-fluid" src="{{ asset($main_blog->category->image ?? '') }}" alt="card image">
                                    @endif
                                    <h3><a href="{{ route('jmor-shows.detail', $main_blog->link) }}">{{ $main_blog->name }}</a></h3>
                        ...
                        <div class="jmor_radio-widget mb-4">
                            ...
                            @foreach($categories as $category)
                                <div class="">
                                    <div class="">
                                        <div class="col-8">
                                            <h6 class="my-2 font-size-14"><a href="{{ route('category-jmor-shows', $category->link) }}">{{ $category->title }}</a></h6>
                                        </div>
                                    </div>
                                </div>
                            @endforeach
