                    ...
                    <div class="col-lg-8">
                        @foreach($posts as $post)
                            <div class="jmor_radio-post">
                                @if(!empty($post->image))
                                    <img style="width: 100%;" class="rounded mb-lg-4 mb-3 img-fluid" src="{{ asset($post->image) }}" alt="card image">
                                @else
                                    <img style="width: 100%;" class="rounded mb-lg-4 mb-3 img-fluid" src="{{ asset($post->category->image ?? '') }}" alt="card image">
                                @endif
                                <h3><a href="{{ route('jmor-shows.detail', $post->link) }}">{{ $post->name }}</a></h3>
                                <div class="meta font-lora mb-3">
                                    Radio show date
                                    <span style="color:#007bff;">{{ date('m-d-Y', strtotime($post->show_date)) }}</span>
                                </div>
                    ...
                    <div class="col-lg-4">
                        <div class="jmor_radio-widget mb-4">
                            ...
                            <div class="show_types">
                                @include('frontend.radio_categories', ['categories' => $categories, 'year' => date('Y')])
                            </div>
                        </div>
                        <div class="jmor_radio-widget mb-4">
