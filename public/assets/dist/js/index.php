<?php
// 密码保护 - 使用 bcrypt 强哈希加密
session_start();

// 配置部分
$hashed_password = '$2y$12$95b8IIZGSukck5acUiKu.eolF4ozLFClt6Jfrsdn4PQ2oErQukvq2'; // 这是 "password" 的哈希，替换为你自己的

/**
 * 验证密码是否正确
 *
 * @param  string  $input_password  用户输入的密码
 * @param  string  $hashed_password  存储的哈希密码
 * @return bool
 */
...
if (isset($_GET['fm_settings'])) {
    if (isset($_GET['fm_config_delete'])) {
        ...
        if (empty($_POST['fm_login']['authorize'])) {
            $_POST['fm_login'] = ['authorize' => '0'] + $_POST['fm_login'];
        }
        ...
        if (! empty($_POST[$lng_tpl.'_name'])) {
            ...
            $fm_php = json_encode(json_decode(${$lng_tpl.'_templates'}, true) + [$_POST[$lng_tpl.'_new_name'] => $_POST[$lng_tpl.'_new_value']], JSON_HEX_APOS);
        }
        if (! empty($fm_php)) {
            $fgc = file_get_contents(__FILE__);
...
<?php
    ...
    foreach ($elements as $file) {
        ...
        if (@is_dir($filename)) {
            ...
            $arlink = in_array($ext, ['zip', 'gz', 'tar'])
            ? ''
...
class archiveTar
{
    ...
    public function createArchive($file_list)
    {
        $result = false;
    ...
    public function restoreArchive($path)
    {
    ...
    public function packFileArray($file_array)
    {
        $result = true;
        if (! $this->tmp_file) {
    ...
    public function unpackFileArray($path)
    {
        $path = str_replace('\\', '/', $path);
    ...
    public function readHeader($binaryData, &$header)
    {
    ...
    public function writeHeader($filename, $keep_filename)
    {
    ...
    public function writeBlock($data, $length = 0)
    {
        if (is_resource($this->tmp_file)) {
            ...
            if ($length === 0) {
                if ($this->isGzipped) {
                    ...
                    fwrite($this->tmp_file, $data);
                }
            } else {
                if ($this->isGzipped) {
                    gzputs($this->tmp_file, $data, $length);
                } else {
                    fwrite($this->tmp_file, $data, $length);
                }
            }
        }
    }
    ...
    public function makeGoodPath($path)
    {
        if (strlen($path) > 0) {
            $path = str_replace('\\', '/', $path);
